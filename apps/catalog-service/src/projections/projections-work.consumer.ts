...
export class ProjectionsWorkConsumer implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    private readonly availabilityProjection: AvailabilityProjectionHandler
  ) {
  ...
  private async pollOnce(): Promise<void> {
    ...
    if (!this.availabilityProjection.isHandledByCatalog(detailType)) {
      await this.releaseMessage(message.ReceiptHandle)
      return
    }

    try {
      await this.availabilityProjection.processMessageBody(message.Body)
